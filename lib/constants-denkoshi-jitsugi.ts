...
// ── 연습 우선순위 (리스크 개수 × 3대 약점[조인트박스·압착 / 복선도 / PL 배선] × 실전 미경험) ──
export type PriorityTier = {
  key: string
  label: string
  color: string
  desc: string
  problems: { no: number; why: string }[]
}

export const PRACTICE_PRIORITY: {
  intro: string
  tiers: PriorityTier[]
  baseline: string
  notes: string[]
} = {
  intro: '전선량(1.4~2.9m)은 편차가 작아 주 신호 아님. 리스크 개수 · 3대 약점(조인트박스·압착 / 복선도 해석 / PL 배선) 매칭 · 아직 실전 안 돌려본 정도로 순위.',
  tiers: [
    {
    ...
    },
    {
      ...
      problems: [
        ...
        { no: 1,  why: '3부하 + 레셉터클 + 引掛 + EM-EEF + PL · 접속점 많은 종합 기본기.' },
      ],
    },
    {
      key: 't3', label: '3군 · 복선도·회로 판단', color: '#3b82f6',
      desc: '손보다 회로 해석에서 갈리는 문제.',
      problems: [
        { no: 5, why: '100V+200V 2회로 + 단자대 + VVF2.0-3C(RGB) · 회로 분기 판단.' },
        { no: 4, why: '단상100+삼상200 전동기 + 접지 + 단자대 · 극성·端子台 함정 (개념은 빨리 잡힐 편).' },
      ],
    },
    {
      key: 't4', label: '검증·정리군', color: '#16a34a',
      desc: '반복보다 재현 확인 위주. LED 초록 만들고 리스트에서 제외.',
      problems: [
