...
create table if not exists kp_ncs (
  ...
  ncs_score   int,                       -- NCS 반영비율 (40 이든 100 이든, 합계로 환산)
  major_label text,                      -- 전공 과목명
  major_score int,                       -- 전공 반영비율
...
-- 반영비율은 문항수와 따로 둔다. 한전KPS 처럼 문항수 50:50 인데 반영이 100:50 인
-- 기업이 있어서, 문항수만으로는 어디에 시간을 쓸지 판단이 안 된다.
-- 값은 공고에 적힌 그대로 넣는다(40/60 이든 100/50 이든). 합계로 나눠 비율을 낸다.
