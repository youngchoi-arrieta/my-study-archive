...
import { Job, CAT_LABELS, STAGES, TRACK_LABELS } from '../types'
...
export default function JobModal({ job, defaultStage = 'watch', onSave, onClose }: Props) {
  ...
  const [track, setTrack] = useState<Job['track']>('job')
  ...
  // 트랙별 안내 문구
  const placeholders = {
    job:         { company: '예: LS electric',     role: '예: 전기설계' },
    scholarship: { company: '예: MEXT, DAAD',      role: '예: Power Engineering' },
    admission:   { company: '예: RWTH Aachen',     role: '예: MSc Electrical Eng.' },
    ra:          { company: '예: ETH PSL Lab',     role: '예: Power Systems RA' },
  }
  const ph = placeholders[track]

  // 직장 트랙일 때만 잡플/블라 점수 노출
  const showRatings = track === 'job'

  async function handleSave() {
    if (!company.trim()) return alert('이름을 입력해주세요')
    setSaving(true)
    await onSave({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-bold mb-4">{titleSuffix} {titlePrefix}</h2>

        {/* 트랙 */}
        <div className="mb-3">
          <label className="text-xs text-gray-500 block mb-1">트랙</label>
          <select className="w-full bg-gray-800 rounded-lg px-3 py-2 text-sm text-white"
            value={track} onChange={e => setTrack(e.target.value as Job['track'])}>
            {(Object.keys(TRACK_LABELS) as Job['track'][]).map(k => (
              <option key={k} value={k}>{TRACK_LABELS[k]}</option>
            ))}
          </select>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div>
            ...
            <input className="w-full bg-gray-800 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600"
              placeholder={ph.company} value={company} onChange={e => setCompany(e.target.value)} />
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">
              {track === 'job' ? '직무' : track === 'scholarship' ? '분야' : '전공'}
            </label>
            <input className="w-full bg-gray-800 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600"
              placeholder={ph.role} value={role} onChange={e => setRole(e.target.value)} />
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            placeholder={
              track === 'job' ? '리뷰 요약, 연봉 정보, 면접 준비 사항 등...' :
              track === 'scholarship' ? '자격 요건, 펀딩 금액, 지원 절차 등...' :
              track === 'admission' ? '교수 컨택 상태, 연구 분야, 입학 요건 등...' :
              '연구 주제, 지도교수, 펀딩 조건 등...'
            }
            value={memo}
