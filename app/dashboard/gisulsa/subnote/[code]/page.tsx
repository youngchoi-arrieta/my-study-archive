...
// 토픽 서브노트 — /dashboard/gisulsa/subnote/[code]
// -------------------------------------------------------------------
// 한 토픽에 한 장. 공통부(원리·도식)를 먼저 쓰고, 시험별 출구를 뒤에 붙인다.
...
import { TOPIC_MAP, GROUP_META, isKoreaOnly } from '@/lib/constants-topics'
...
export default function SubnotePage() {
  ...
  const topic = TOPIC_MAP.get(code)
  ...
  const fetchAll = useCallback(async () => {
    ...
    setKr(all.filter(q => q.topics.includes(topic.code)))
    setJp(refs.get(topic.code) ?? [])
    const n = notes.get(topic.code)
    setBody(n?.body ?? '')
  ...
  const persist = async (patch: { body?: string; status?: number }) => {
    const err = await saveSubnote(topic.code, patch)
    ...
    <main className="min-h-screen bg-gray-950 text-white p-6 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-1">
          <span className="text-xs font-bold px-2 py-0.5 rounded font-mono text-white" style={{ backgroundColor: accent }}>
            {topic.code}
          </span>
          <h1 className="text-2xl font-bold">{topic.name}</h1>
        </div>
        <p className="text-gray-500 text-sm mb-1">
          {topic.group} · 기술사 {kr.length}문 {krPoints}점 · 電験 {jp.length}문
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-900 rounded-2xl p-4">
            ...
            {kr.length === 0 ? (
              <p className="text-[11px] text-gray-600">이 토픽으로 태깅된 기술사 문항이 없습니다.</p>
            ) : (
