...
type ProblemRow = { track: string; status: ResearchStatus }
...
export default function ResearchHub() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data } = await supabase.from('research_problems').select('track, status')
  ...
  const trackStats = useMemo(() => {
    ...
    for (const t of RESEARCH_TRACKS) m.set(t.slug, { untouched: 0, studying: 0, understood: 0 })
    for (const p of problems) {
      const rec = m.get(p.track)
  ...
  return (
    <main className="min-h-screen bg-[#050d1a] text-white p-5 md:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-gray-600 text-sm mb-6">電験 1·2종 / 기술고시 — 점수가 아니라 이해도로 추적하는 논술·심화 기출 연구</p>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {RESEARCH_TRACKS.map(track => {
              ...
              const total = stats.untouched + stats.studying + stats.understood
              const done = stats.understood
              const studying = stats.studying

              return (
                <button key={track.slug} onClick={() => router.push(`/dashboard/research/${track.slug}`)}
                  ...
                  {total > 0 ? (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2 text-[11px] shrink-0">
                        ...
                        <span className="text-gray-500">{total}문제</span>
                        {studying > 0 && <span className="text-yellow-400">· 연구중 {studying}</span>}
                      </div>
                    </div>
                  ) : (
                    <p className="text-[11px] text-gray-700">아직 등록된 문제 없음</p>
                  )}
                </button>
              )
            })}
