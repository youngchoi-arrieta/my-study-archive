...
export default function ChapterProblemGrid() {
  ...
  const chapter = subject ? getChapter(subject, chapterSlug) : undefined
  ...
  const loadData = useCallback(async () => {
    ...
    setLoaded(true)
  }, [chapter, chapterSlug, subjectSlug])
      ...
      <main className="min-h-screen bg-[#050d1a] text-white flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-500 mb-3">
            {subject ? '단원을 찾을 수 없어요.' : '등록되지 않은 과목이다.'}
          </p>
          <Link href={subject ? `/dashboard/textbook/${subjectSlug}` : '/dashboard/textbook'}
            className="text-blue-400 hover:underline text-sm">
            ← {subject ? subject.name : 'N제 교재'}
          </Link>
        </div>
      </main>
    )
  ...
  return (
    <main className="min-h-screen bg-[#050d1a] text-white flex flex-col" style={{ height: '100dvh' }}>
      ...
      <div className="shrink-0 bg-[#0a1628] border-b border-white/5 px-3 py-2 flex items-center gap-3">
        <Link href={`/dashboard/textbook/${subjectSlug}`} className="text-gray-500 hover:text-white text-xs transition">← {subject.name}</Link>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-40 shrink-0 bg-[#080f1e] border-r border-white/5 overflow-y-auto py-2">
          {subject.chapters.map(ch => {
            ...
            return (
              <button key={ch.slug}
                onClick={() => router.push(`/dashboard/textbook/${subjectSlug}/${ch.slug}`)}
